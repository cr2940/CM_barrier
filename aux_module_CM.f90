...
contains
    ...
    subroutine cm_update_fluc(fm,fm2,gm,gm2,fp,fp2,gp,gp2,gradQ,gradQ2,ii,jj,N_cells,&
      ...
      ecen_up_x,ecen_up_y,lengths_sunder,lengths_supper,&
      mthlim,ot)
      ...
      real(8) fp(3,-1:max(mx,my)+2,-1:max(mx,my)+2)
      real(8) fm(3,-1:max(mx,my)+2,-1:max(mx,my)+2)
      real(8) gp(3,-1:max(mx,my)+2,-1:max(mx,my)+2)
      real(8) gm(3,-1:max(mx,my)+2,-1:max(mx,my)+2)
      real(8) fp2(3,-1:max(mx,my)+2,-1:max(mx,my)+2)
      real(8) fm2(3,-1:max(mx,my)+2,-1:max(mx,my)+2)
      real(8) gp2(3,-1:max(mx,my)+2,-1:max(mx,my)+2)
      real(8) gm2(3,-1:max(mx,my)+2,-1:max(mx,my)+2)
      ...
      real(8) :: aux_un,aux_up,wall_height,wall_ht2,temp_dist(2)
      ...
      real(8) :: amdq_c(3),apdq_c(3),wave_wall(3,3),s_wall(3)
      logical :: ot
        do i=1,N_cells
          i_0 = ii(i)
          j_0 = jj(i)
          ...
          ! barrier height:
          ! print*, "bar ht ",wall_height

          wall_ht2 = wall_height!-0.8/10000 - aux(1,i_0,j_0) + wall_height
        ...
        (area_supper(i)*qold2(:,i_0,j_0)+up_area_ij(i_0,j_0+1)*qold2(:,i_0,j_0+1))
             ...
             call rotated_redist_fwave(q1,q1l,aux_up,aux_un, &
                  x,y,amdq_wall,apdq_wall,2,wall_ht2,wave_wall,s_wall,ot)
              ...
              if (order .eq. 2) then
                ...
                call riemann_solver(qold2(:,i_0-2:i_0,j_0),qold2(:,i_0-1:i_0+1,j_0),&
                   auxu(1,i_0-2:i_0,j_0),auxu(1,i_0-1:i_0+1,j_0),s,fwave,&
                    amdq,apdq,1,1,dt/dx,mthlim)
                fwave(:,:,3) = wave_wall
                s(:,3) = s_wall * lengths_supper(1,i)
                call wave_lim(fwave,s,mthlim,dt/dx,amdq,apdq)
                apdq_c = apdq(:,2)
              else
                apdq_c = fp2(:,i_0,j_0)
              end if
          ...
          case(3,6)
            ...
            if (order .eq. 2) then
              ...
              call rotated_redist_fwave(q1l,q1,aux_up,aux_un, &
                    x,y,amdq_wall,apdq_wall,ixy,wall_ht2,wave_wall,s_wall,ot)
            ...
            ! the upper edge
                if (order .eq. 2) then
                  call riemann_solver(qold2(:,i_0,j_0-1:j_0+1),qold2(:,i_0,j_0:j_0+2),&
                    ...
                    amdq,apdq,2,1,dt/dx,mthlim)
                  fwave(:,:,1) = wave_wall
                  s(:,1) = s_wall * lengths_supper(1,i)
                  call wave_lim(fwave,s,mthlim,dt/dx,amdq,apdq)
                  amdq_c = amdq(:,2)
                else
        ...
        (area_supper(i)*qold2(:,i_0,j_0)+up_area_ij(i_0,j_0-1)*qold2(:,i_0,j_0-1))
             ...
             call rotated_redist_fwave(q1,q1l,aux_up,aux_un, &
                  x,y,amdq_wall,apdq_wall,1,wall_ht2,wave_wall,s_wall,ot)
             cm_avg2 = cm_avg2 - dt/dx *lengths_supper(1,i)*(1/(area_supper(i)+&
               up_area_ij(i_0,j_0-1))) *amdq_wall
              ...
              call rotated_redist_fwave(q1,q1l,aux_un,aux_up, &
                   x,y,apdq_wall,amdq_wall,1,wall_ht2,wave_wall,s_wall,ot)
              cm_avg2 = cm_avg2 - dt/dx *lengths_supper(1,i+1)*(1/(area_supper(i)+&
                up_area_ij(i_0,j_0-1))) *amdq_wall
             ...
             if (order .eq. 2) then
              call riemann_solver(qold2(:,i_0-1:i_0+1,j_0-1),qold2(:,i_0:i_0+2,j_0-1),&
                ...
                amdq,apdq,1,1,dt/dx,mthlim)
              fwave(:,:,1) = wave_wall
              s(:,1) = s_wall * lengths_supper(1,i+1)
              call wave_lim(fwave,s,mthlim,dt/dx,amdq,apdq)
              amdq_c = amdq(:,2)
            else
              amdq_c = fm2(:,i_0+1,j_0-1)
                ...
                if (order .eq. 2) then
                  if(type_supper(i) .eq. 2) then
                    ...
                    call riemann_solver(qold2(:,i_0-1:ilat,j_0),qold2(:,i_0:ilat+1,j_0),&
                       auxu(1,i_0-1:ilat,j_0),auxu(1,i_0:ilat+1,j_0),s,fwave,&
                      amdq,apdq,1,1,dt/dx,mthlim )
                    fwave(:,:,1) = wave_wall
                    s(:,1) = s_wall * lengths_supper(1,i)
                    call wave_lim(fwave,s,mthlim,dt/dx,amdq,apdq)
...
!!!!!!!!!!!!!!!!!!!!!!!!   ! FOR LOWER CELLS  !!!!!!!!!!!!!!!!!!!
              ...
              if (order .eq. 2) then
                call riemann_solver(qold(:,i_0-2:i_0,j_0+1),qold(:,i_0-1:i_0+1,j_0+1),&
                 aux(1,i_0-2:i_0,j_0+1),aux(1,i_0-1:i_0+1,j_0+1),s,fwave,&
                    amdq,apdq,1,1,dt/dx,mthlim)
                fwave(:,:,3) = wave_wall
                s(:,3) = s_wall * lengths_sunder(1,i-1)
                call wave_lim(fwave,s,mthlim,dt/dx,amdq,apdq)
                apdq_c =apdq(:,2)
              else
                apdq_c = fp(:,i_0,j_0+1)
    ...
    subroutine rotated_redist_fwave(ql,qr,aux_up,aux_un,x,&
          y,amdq_wall,apdq_wall,ixy,wall_height,wave_wall,s_wall,ot)
      ...
      ot = L2R .or. R2L
       ...
       if (ixy.eq.1) then
        ...
        do i=1,3
          call rotate_state(wave_wall(:,i),wave_wall(:,i),t_vec,n_vec2)
        end do
      else
        call rotate_state(amdq_wall,amdq_wall,-t_vec,-n_vec2)
        call rotate_state(apdq_wall,apdq_wall,-t_vec,-n_vec2)
    ...
    subroutine grad_calc(ii,jj,mx,my,dx,dy,N_cells,gradQ,gradQ2,qold,qold2,type_supper,&
      ...
      do i = 1, N_cells
        ...
        case(4)
          ...
          gradQ2(:,:,i_0,j_0) = nabla_Q(delQ1,delR1,1,dx,qold2(:,i_0,j_0))
          ...
          gradQ(:,:,i_0,j_0) = nabla_Q(delQ2,delR2,2,dx,qold(:,i_0,j_0))
        ...
        case(3,6)
          ...
          gradQ(:,:,i_0,j_0) = nabla_Q(delQ1,delR1,1,dx,qold(:,i_0,j_0))
        case(1)
          ! gradQ2(:,:,i_0,j_0+1) = nabla_Q_normal(qold2,cen_grid_up,&
          ...
          gradQ2(:,:,i_0,j_0) = nabla_Q(delQ2,delR2,2,dx,qold2(:,i_0,j_0))
          delQ1(:,1) = qold(:,i_0+1,j_0)-qold(:,i_0,j_0)
    ...
    function nabla_Q(delQ,delR,ixy,dx,q) result(nabla)
      implicit none
      real(8) :: delQ(:,:) , delR(:,:)
      real(8) :: nabla(3,2),dx,q(3),alpha(3)
    ...
    alpha = BJ_limiter(delQ,q)
    nabla(:,1) = nabla(:,1) * alpha
    nabla(:,2) = nabla(:,2) * alpha
  end function nabla_Q

    function BJ_limiter(delQ,q) result(alpha)
      implicit none
      real(8) :: delQ(:,:), q(3)
      real(8) :: alpha(3), temp1, temp2
      integer :: i, j, N
      real(8) :: dq_max(3),dq_min(3)

      N = size(delQ,2)
      dq_max = (/maxval(delQ(1,:)),maxval(delQ(2,:)),maxval(delQ(3,:))/)
      dq_min = (/minval(delQ(1,:)),minval(delQ(2,:)),minval(delQ(3,:))/)

      do i = 1,3
        temp2 = 1.d0
        do j = 1, N
          if (delQ(i,j) > 0.d0) then
            temp1 = min(1.d0, dq_max(i)/ delQ(i,j))
          else if (delQ(i,j) < 0.d0) then
            temp1 = min(1.d0, dq_min(i) / delQ(i,j))
          else
            temp1 = 1.d0
          end if
    ...
    function nabla_Q_normal(qold,cen_grid,i,j,mx,my,dx,dy) result(nabla)
      ...
      nabla = nabla_Q(delQ,delR,2,dx,qold(:,i,j))
