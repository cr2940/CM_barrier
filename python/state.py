...
class State(object):
    ...
    :State Data:
        ...
        make another q2, auxu, for upper small cell values!
    ...
    def __init__(self,geom,num_eqn,num_aux=0):
        ...
        self.q2  = self.new_array(num_eqn)
        self.auxu = self.new_array(num_aux)
    ...
    def is_valid(self):
        ...
        if self.aux is not None:
            if not self.aux.flags['F_CONTIGUOUS']:
                ...
                valid = False
        if not self.q2.flags['F_CONTIGUOUS']:
            logger.debug('q2 array is not Fortran contiguous.')
            valid = False
        if self.auxu is not None:
            if not self.auxu.flags['F_CONTIGUOUS']:
                logger.debug('auxu array is not Fortran contiguous.')
                valid = False
        return valid
    ...
    def set_q_from_qbc(self,num_ghost,qbc,qbc2):
        """
        ...
        if num_dim == 1:
            self.q2 = qbc2[:,num_ghost:-num_ghost]
        elif num_dim == 2:
            self.q2 = qbc2[:,num_ghost:-num_ghost,num_ghost:-num_ghost]
        elif num_dim == 3:
            self.q2 = qbc2[:,num_ghost:-num_ghost,num_ghost:-num_ghost,num_ghost:-num_ghost]
        else:
            raise Exception("Assumption (1 <= num_dim <= 3) violated.")

    def set_aux_from_auxbc(self,num_ghost,auxbc,auxbc2):
        """
        ...
        if patch.num_dim == 1:
            ...
            raise Exception("Assumption (1 <= num_dim <= 3) violated.")
        if patch.num_dim == 1:
            self.auxu = auxbc2[:,num_ghost:-num_ghost]
        elif patch.num_dim == 2:
            self.auxu = auxbc2[:,num_ghost:-num_ghost,num_ghost:-num_ghost]
        elif patch.num_dim == 3:
            self.auxu = auxbc2[:,num_ghost:-num_ghost,num_ghost:-num_ghost,num_ghost:-num_ghost]
        else:
            raise Exception("Assumption (1 <= num_dim <= 3) violated.")


    def get_qbc_from_q(self,num_ghost,qbc,qbc2):
        """
        ...
        if num_dim == 1:
            ...
            raise Exception("Assumption (1 <= num_dim <= 3) violated.")
        if num_dim == 1:
            qbc2[:,num_ghost:-num_ghost] = self.q2
        elif num_dim == 2:
            qbc2[:,num_ghost:-num_ghost,num_ghost:-num_ghost] = self.q2
        elif num_dim == 3:
            qbc2[:,num_ghost:-num_ghost,num_ghost:-num_ghost,num_ghost:-num_ghost] = self.q2
        else:
            raise Exception("Assumption (1 <= num_dim <= 3) violated.")

        return qbc,qbc2

    def get_auxbc_from_aux(self,num_ghost,auxbc,auxbc2):
        """
        ...
        if num_dim == 1:
            auxbc2[:,num_ghost:-num_ghost] = self.auxu
        elif num_dim == 2:
            auxbc2[:,num_ghost:-num_ghost,num_ghost:-num_ghost] = self.auxu
        elif num_dim == 3:
            auxbc2[:,num_ghost:-num_ghost,num_ghost:-num_ghost,num_ghost:-num_ghost] = self.auxu
        else:
            raise Exception("Assumption (1 <= num_dim <= 3) violated.")
    ...
    def __deepcopy__(self,memo={}):
        ...
        if self.q2 is not None:
            result.q2 = copy.deepcopy(self.q2)
        if self.auxu is not None:
            result.auxu = copy.deepcopy(self.auxu)
    ...
    def get_q_global(self):
        ...
        """
        return self.q.copy(),self.q2.copy()

    def get_aux_global(self):
        r"""
        Returns a copy of state.aux.
        """
        return self.aux.copy(),self.auxu.copy()
